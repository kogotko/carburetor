...
class VolTypeExtrasTests(test.BaseAdminViewTests):
    ...
    def test_list_extras_when_none_exists(self):
        ...
        self.mox.ReplayAll()
        url = reverse('horizon:uhd:volumes:volume_types:extras:index',
                      args=[vol_type.id])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp,
                                "uhd/volumes/volume_types/extras/index.html")
    ...
    def test_extra_create_post(self):
        vol_type = self.cinder_volume_types.first()
        create_url = reverse(
            'horizon:uhd:volumes:volume_types:extras:create',
            args=[vol_type.id])
        index_url = reverse(
            'horizon:uhd:volumes:volume_types:extras:index',
            args=[vol_type.id])
    ...
    def test_extra_create_get(self):
        ...
        self.assertTemplateUsed(
            resp, 'uhd/volumes/volume_types/extras/create.html')
    ...
    def test_extra_edit(self):
        vol_type = self.cinder_volume_types.first()
        key = 'foo'
        edit_url = reverse('horizon:uhd:volumes:volume_types:extras:edit',
                           args=[vol_type.id, key])
        index_url = reverse('horizon:uhd:volumes:volume_types:extras:index',
                            args=[vol_type.id])
