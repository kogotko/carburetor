...
from openstack_dashboard.dashboards.uhd.volumes.volumes \
    import forms as volumes_forms
from openstack_dashboard.dashboards.uhd.volumes.volumes \
    import tables as volumes_tables
from openstack_dashboard.dashboards.uvr.volumes.volumes \
...
class DetailView(volumes_views.DetailView):
    ...
    def get_redirect_url(self):
        return reverse('horizon:uhd:volumes:index')


class ManageVolumeView(forms.ModalFormView):
    form_class = volumes_forms.ManageVolume
    template_name = 'uhd/volumes/volumes/manage_volume.html'
    form_id = "manage_volume_modal"
    submit_label = _("Manage")
    success_url = reverse_lazy('horizon:uhd:volumes:volumes_tab')
    submit_url = reverse_lazy('horizon:uhd:volumes:volumes:manage')
    cancel_url = reverse_lazy("horizon:uhd:volumes:index")
...
class UnmanageVolumeView(forms.ModalFormView):
    ...
    template_name = 'uhd/volumes/volumes/unmanage_volume.html'
    form_id = "unmanage_volume_modal"
    submit_label = _("Unmanage")
    success_url = reverse_lazy('horizon:uhd:volumes:volumes_tab')
    submit_url = 'horizon:uhd:volumes:volumes:unmanage'
    cancel_url = reverse_lazy("horizon:uhd:volumes:index")
...
class MigrateVolumeView(forms.ModalFormView):
    ...
    template_name = 'uhd/volumes/volumes/migrate_volume.html'
    form_id = "migrate_volume_modal"
    submit_label = _("Migrate")
    success_url = reverse_lazy('horizon:uhd:volumes:volumes_tab')
    submit_url = 'horizon:uhd:volumes:volumes:migrate'
    cancel_url = reverse_lazy("horizon:uhd:volumes:index")
...
class UpdateStatusView(forms.ModalFormView):
    ...
    template_name = 'uhd/volumes/volumes/update_status.html'
    submit_label = _("Update Status")
    submit_url = "horizon:uhd:volumes:volumes:update_status"
