...
from openstack_dashboard.dashboards.uhd.volumes.snapshots import forms

INDEX_URL = reverse('horizon:uhd:volumes:volumes_tab')
...
class VolumeViewTests(test.BaseAdminViewTests):
    ...
    def test_update_volume_status(self):
        ...
        res = self.client.post(
            reverse('horizon:uhd:volumes:volumes:update_status',
                    args=(volume.id,)),
            formData)
    ...
    def test_manage_volume(self):
        ...
        self.mox.ReplayAll()
        res = self.client.post(
            reverse('horizon:uhd:volumes:volumes:manage'),
            formData)
        self.assertNoFormErrors(res)
    ...
    def test_unmanage_volume(self):
        ...
        self.mox.ReplayAll()
        res = self.client.post(
            reverse('horizon:uhd:volumes:volumes:unmanage',
                    args=(volume.id,)),
            formData)
        self.assertNoFormErrors(res)
    ...
    def test_volume_migrate_get(self):
        ...
        url = reverse('horizon:uhd:volumes:volumes:migrate',
                      args=[volume.id])
        res = self.client.get(url)

        self.assertTemplateUsed(res,
                                'uhd/volumes/volumes/migrate_volume.html')
    ...
    def test_update_volume_status_get(self):
        ...
        url = reverse('horizon:uhd:volumes:volumes:update_status',
                      args=[volume.id])
        res = self.client.get(url)
